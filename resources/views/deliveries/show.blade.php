                ...
                <div class="row mb-4">
                    <div class="col-md-6">
                        <div class="border rounded p-3 h-100">
                            <h6 class="text-primary"><i class="bi bi-person-up me-1"></i>{{ __('messages.sender') }}</h6>
                            <p class="mb-1"><strong>{{ $delivery->sender_name }}</strong></p>
                            <p class="mb-1">{{ $delivery->sender_phone }}</p>
                            <p class="mb-0"><small class="text-muted">{{ __('messages.from') }}:</small><br>{{ $delivery->sender_address }}</p>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="border rounded p-3 h-100">
                            <h6 class="text-success"><i class="bi bi-person-down me-1"></i>{{ __('messages.receiver') }}</h6>
                            <p class="mb-1"><strong>{{ $delivery->receiver_name }}</strong></p>
                            <p class="mb-1">{{ $delivery->receiver_phone }}</p>
                            <p class="mb-0"><small class="text-muted">{{ __('messages.to') }}:</small><br>{{ $delivery->receiver_address }}</p>
                        </div>
                    </div>
                </div>
            ...
            <div class="card-body">
                <form method="POST" action="{{ route('deliveries.update-status', $delivery) }}">
                    ...
                    <select name="status" class="form-select mb-3" required>
                        <option value="pending" {{ $delivery->status === 'pending' ? 'selected' : '' }}>{{ __('messages.pending') }}</option>
                        <option value="picked_up" {{ $delivery->status === 'picked_up' ? 'selected' : '' }}>{{ __('messages.picked_up') }}</option>
                        <option value="in_transit" {{ $delivery->status === 'in_transit' ? 'selected' : '' }}>{{ __('messages.in_transit') }}</option>
                        <option value="delivered" {{ $delivery->status === 'delivered' ? 'selected' : '' }}>{{ __('messages.delivered') }}</option>
                        <option value="cancelled" {{ $delivery->status === 'cancelled' ? 'selected' : '' }}>{{ __('messages.cancelled') }}</option>
                    </select>
